  ...
  - Bash scripts and commands: Use the #!BASH marker in your execute block for both simple commands and complex shell scripts with variables, loops, conditionals, etc.
    ...
    def configure(self, self_critic=False, test_time_scale_round=0):
        ...
        def generate(state: AgentState) -> AgentState:
            ...
            # First, normalize incomplete tags (GLM-4.7 often returns </execute without >)
            # Match </execute or </execute> followed by anything except >
            msg = re.sub(r'</execute\>?(?!\s*[>])', '</execute>', msg)
            msg = re.sub(r'</execute(?![>])', '</execute>', msg)
            msg = re.sub(r'</solution\>?(?!\s*[>])', '</solution>', msg)
            msg = re.sub(r'</solution(?![>])', '</solution>', msg)
        ...
        def execute(state: AgentState) -> AgentState:
            ...
            # First, normalize incomplete tags (GLM-4.7 often returns </execute without >)
            # Match </execute or </execute> followed by anything except >
            last_message = re.sub(r'</execute\>?(?!\s*[>])', '</execute>', last_message)
            last_message = re.sub(r'</execute(?![>])', '</execute>', last_message)
            last_message = re.sub(r'</solution\>?(?!\s*[>])', '</solution>', last_message)
            last_message = re.sub(r'</solution(?![>])', '</solution>', last_message)
            ...
            if execute_match:
                ...
                # Check if the result contains various package installation error patterns
                # Ask LLM to provide BOTH conda and pip install commands
                # Try conda first (with verification), then pip (with verification)
                # ============================================================
                error_patterns = [
                    r"No module named ['\"]([^'\"]+)['\"]",  # Standard Python import error
                    r"Error: Please install (\S+) package via",  # skmisc style: "Please install xxx package via"
                    r"Error: Please install the (\S+) package",  # igraph style: "Please install the xxx package"
                    r"Error: Please install the (\S+) algorithm",  # leidenalg style: "Please install the xxx algorithm"
                    r"Please install (\S+):",  # Generic: "Please install xxx:"
                    r"install (`[^`]+`)",  # Capture package name in backticks
                ]
                ...
                for pattern in error_patterns:
                    ...
                    if match:
                        missing_package = match.group(1).strip('`\'"')
                        print(f"[AUTO-INSTALL] Detected missing package: {missing_package}")
                        break
...
Respond in this exact format (one command per line):
CONDA: conda install -y <package_name>
PIP: pip install <package_name>

Only provide these two lines, no explanation."""
                        ...
                        try:
                            ...
                            failure_reasons.append("Conda: Installation timed out")
                        except Exception as e:
                            failure_reasons.append(f"Conda: {str(e)}")

                        # If conda failed, try pip
                        if not installed:
                            ...
                            try:
                                ...
                                failure_reasons.append("Pip: Installation timed out")
                            except Exception as e:
                                failure_reasons.append(f"Pip: {str(e)}")

                        # Retry if installation succeeded
                        if installed:
                            print(f"[AUTO-INSTALL] Successfully installed {missing_package}")
                            print(f"[AUTO-INSTALL] Retrying code execution...")

                            # Re-run the same code
                            ...
                            print(f"[AUTO-INSTALL] Retry completed")
                        else:
                            print(f"[AUTO-INSTALL] All installation methods failed")
                            error_msg = f"\n\n[NOTE] Failed to install '{missing_package}'.\n"
                        ...
                        import traceback
                        traceback.print_exc()
                        result += f"\n\n[NOTE] Failed to install '{missing_package}'. Exception: {e}"
