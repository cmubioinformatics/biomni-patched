...
def create_task_dir(task_name: str, base_dir: str = "./outputs") -> str:
    """Create a task-specific directory for organizing output files.

    Automatically generates a directory name with timestamp to avoid conflicts.

    Args:
        task_name: Brief description of the task (e.g., "literature_search", "data_analysis")
        base_dir: Base directory for task outputs (default: "./outputs")

    Returns:
        str: Full path to the created task directory

    Example:
        >>> dir_path = create_task_dir("pubmed_search")
        >>> print(dir_path)
        ./outputs/20250124_143022_pubmed_search
    """
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    # Clean task name: replace spaces with underscores, remove special chars
...
def run_python_repl(command: str, working_dir: str = None) -> str:
    ...
    Args:
        ...
        working_dir: Optional directory to execute in (creates if doesn't exist)

    Returns:
        str: Output from command execution
    """

    def execute_in_repl(command: str, working_dir: str = None) -> str:
        ...
        try:
            ...
            # Update working directory in namespace
            if working_dir:
                _persistent_namespace['__task_dir__'] = working_dir
